...
/* ethernet wrapper with AXI interface
 *
 * Memory map:
...
module ether_rmii #(
    ...
    input  wire                     crs_dv_i       ,
    input  wire                     rxerr_i        ,
    input  wire                [1:0] rxd_i         ,
    output wire                     txen_o         ,
    output wire                [1:0] txd_o         ,
    input  wire                     intn_i
);

    // DRC: design rule check
    initial begin
        if (DATA_WIDTH!=32) $fatal(1, "ether wrapper supports only DATA_WIDTH=32");
        if (ADDR_WIDTH<16) $fatal(1, "ether wrapper requires ADDR_WIDTH >= 16");
    end
    ...
    wire rst_50mhz_i = rst_i;
    wire intn_unused = intn_i;

    reg [9:0] rst_wait;

    // NOTE: MDIO/INT is reserved for future use in this wrapper.
    assign mdio_io   = 1'bz;
    assign mdc_o     = 1'b0;
    assign rstn_o    = (rst_wait==10'h3FF);

    always @(posedge clk_50mhz_i) begin
        if (rst_i) begin
            rst_wait <= 10'h000;
        end else if (rst_wait!=10'h3FF) begin
    ...
    // RX buffer RAM (ETH clock -> CPU clock)
    wire                        rx_data_valid;
    ...
    // TX buffer RAM (CPU clock -> ETH clock)
