...
/* ethernet receiver (MII 4-bit) */
/******************************************************************************************/
module ether_rx_mii #(
    parameter  DATA_WIDTH        = 32  , // data width
    parameter  BUFFER_ADDR_WIDTH = 14    // RX buffer address width
) (
    input  wire                         rx_clk_i            , // MII RX clock
    input  wire                         rst_rx_i            , // reset (rx clock domain)
    ...
    // Ethernet PHY (MII)
    input  wire                         rx_dv_i             , // RX data valid
    input  wire                         rxerr_i             , // receive error
    input  wire                    [3:0] rxd_i               // receive data nibble
);

    // DRC: design rule check
    initial begin
        if (DATA_WIDTH!=32) $fatal(1, "ether_rx_mii supports only DATA_WIDTH=32");
        if (BUFFER_ADDR_WIDTH<2) $fatal(1, "specify proper ether_rx_mii BUFFER_ADDR_WIDTH");
    end
    ...
    localparam ETHER_FIN_CYCLES  = 9;  // tuned for 4-bit MII stream
    localparam WAIT_CYCLES       = 24-ETHER_FIN_CYCLES-1;
    ...
    always @(*) begin
        ...
        if (rx_dv_i) begin
            // place each 4-bit MII nibble to the word shift register
            rx_data_d = {rxd_i, rx_data_d[DATA_WIDTH-1:4]};
        end
        ...
        case (state_q)
            IDLE: begin
                ...
                end else if (rx_dv_i && !rxerr_i) begin
                    rx_addr_origin_d = rx_addr_idx_q & BUFFER_CEIL;
                    rx_addr_idx_d    = rx_addr_idx_q & BUFFER_CEIL;
                    preamble_cnt_d   = (rxd_i==4'h5) ? preamble_cnt_q+'h1 : 6'h0;
                    if (preamble_cnt_q>=6'd6) begin
                        rx_ready_d = 1'b1;
                    end
                    nibble_cnt_d = ~nibble_cnt_q;
                    if (rx_ready_q && ({rxd_i, rx_data_q[7:4]}==SFD)) begin
            ...
            RECEIVE: begin
                if (rx_dv_i && !rxerr_i) begin
                    fin_cycle_cnt_d = 6'h0;
                    if (crc_valid_q && !drop_frame_q) begin
                        crc_frame_d = crc32_next_nibble(crc_frame_q, rx_data_q[3:0]);
                    end
                    if (nibble_cnt_q) begin
                        nibble_cnt_d     = 1'b0;
                        dst_byte_now     = {rxd_i, rx_data_q[31:28]};
