...
# Pblock
#create_pblock pb0;
#add_cells_to_pblock [get_pblocks pb0] [get_cells -quiet [list bootrom clint cpu dram_controller mmu plic axi_interconnect uart]];
...
set_property -dict { PACKAGE_PIN G18   IOSTANDARD LVCMOS33 } [get_ports { eth_refclk }]; #IO_L22P_T3_A17_15 Sch=eth_ref_clk
...
# MII reference clock output (FPGA -> PHY)
create_clock -period 40.000 -name eth_refclk_pin -waveform {0.000 20.000} [get_ports eth_refclk]

# MII Timing Constraints (100Mbps => 25MHz)
create_clock -period 40.000 -name eth_rx_clk_pin -waveform {0.000 20.000} [get_ports eth_rx_clk]
create_clock -period 40.000 -name eth_tx_clk_pin -waveform {0.000 20.000} [get_ports eth_tx_clk]

# Input delays for receive signals (from PHY to FPGA)
set_input_delay -clock [get_clocks eth_rx_clk_pin] -max 8.000 [get_ports {eth_rxd[*]}]
set_input_delay -clock [get_clocks eth_rx_clk_pin] -min 0.000 [get_ports {eth_rxd[*]}]
set_input_delay -clock [get_clocks eth_rx_clk_pin] -max 8.000 [get_ports eth_rx_dv]
set_input_delay -clock [get_clocks eth_rx_clk_pin] -min 0.000 [get_ports eth_rx_dv]
set_input_delay -clock [get_clocks eth_rx_clk_pin] -max 8.000 [get_ports eth_rxerr]
set_input_delay -clock [get_clocks eth_rx_clk_pin] -min 0.000 [get_ports eth_rxerr]

# Output delays for transmit signals (from FPGA to PHY)
set_output_delay -clock [get_clocks eth_tx_clk_pin] -max 4.000 [get_ports {eth_txd[*]}]
set_output_delay -clock [get_clocks eth_tx_clk_pin] -min -2.000 [get_ports {eth_txd[*]}]
set_output_delay -clock [get_clocks eth_tx_clk_pin] -max 4.000 [get_ports eth_tx_en]
set_output_delay -clock [get_clocks eth_tx_clk_pin] -min -2.000 [get_ports eth_tx_en]
...
set_clock_groups -asynchronous -group [get_clocks -of_objects [get_pins clk_wiz_1/inst/mmcm_adv_inst/CLKOUT0]] -group [get_clocks eth_rx_clk_pin];
set_clock_groups -asynchronous -group [get_clocks -of_objects [get_pins clk_wiz_1/inst/mmcm_adv_inst/CLKOUT0]] -group [get_clocks eth_tx_clk_pin];
set_clock_groups -asynchronous -group [get_clocks eth_rx_clk_pin] -group [get_clocks eth_tx_clk_pin];
